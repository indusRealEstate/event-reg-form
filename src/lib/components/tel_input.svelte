<script lang="ts">
	...
	let searchText = '';
	let isOpen = false;
	export let selectedCountry: CountryCode = 'GB';
	...
	const selectClick = () => {
		if (closeOnClick) closeDropdown(null);
	};
...
>
	<div class="flex" use:clickOutsideAction={closeOnClickOutside}>
		<button
			...
			class="relative flex-shrink-0 overflow-hidden z-10 whitespace-nowrap inline-flex items-center py-2.5 px-4 text-sm font-medium text-center text-gray-500 border border-gray-300 rounded-l-lg hover:bg-gray-200 focus:outline-none  dark:border-gray-600"
			type="button"
			role="combobox"
